...
// CodeGuideModel is the Bubble Tea model for the interactive code-guide UI.
...
func NewCodeGuideModel(gemini *core.GeminiChat) *CodeGuideModel {
	...
	ti.Placeholder = "Ask Gemini about the codebase..."
	ti.Focus()
	ti.CharLimit = 256
	ti.Width = 80
	ti.Prompt = "> "
...
func (m CodeGuideModel) Init() tea.Cmd {
	return tea.Batch(textinput.Blink, m.spinner.Tick)
}

// Update handles messages and updates the model.
func (m CodeGuideModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	var cmd tea.Cmd
	var cmds []tea.Cmd
	...
	case tea.KeyMsg:
		...
		case tea.KeyEnter:
			...
			return m, tea.Batch(append(cmds, m.codeGuideCmd(query))...)
		case tea.KeyCtrlC, tea.KeyEsc:
			return m, tea.Quit
		}
	case codeGuideResponseMsg:
...
// View renders the UI.
func (m CodeGuideModel) View() string {
	s := strings.Builder{}
...
// codeGuideCmd is a command that sends the query to Gemini.
func (m CodeGuideModel) codeGuideCmd(query string) tea.Cmd {
	return func() tea.Msg {
		...
		return codeGuideResponseMsg{Value: content}
	}
}

// codeGuideResponseMsg is a message type for Gemini's response.
type codeGuideResponseMsg struct {
