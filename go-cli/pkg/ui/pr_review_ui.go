...
// PrReviewModel is the Bubble Tea model for the interactive pr-review UI.
...
func NewPrReviewModel(gemini core.Executor) *PrReviewModel {
	...
	ti.Placeholder = "Enter PR identifier..."
	ti.Focus()
	ti.CharLimit = 256
	ti.Width = 80
	ti.Prompt = "> "
...
func (m PrReviewModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case tea.KeyEnter:
			...
			return m, tea.Batch(append(cmds, m.prReviewCmd(query))...)
		case tea.KeyCtrlC, tea.KeyEsc:
			return m, tea.Quit
		}
	case prReviewResponseMsg:
...
// prReviewCmd is a command that sends the query to Gemini.
func (m PrReviewModel) prReviewCmd(query string) tea.Cmd {
	return func() tea.Msg {
		...
		return prReviewResponseMsg{Value: content}
	}
}

// prReviewResponseMsg is a message type for Gemini's response.
type prReviewResponseMsg struct {
